...
5. **之後每次回來**：看工具列上的 `⏱ Last download` 標籤，
   再點 `🆕 Select updated`，就能把該基準時間之後有更新、且目前可見的
   課程全部加入你目前的勾選（詳見下方
   [上次下載追蹤](#上次下載追蹤)）
...
- **`⏱ Last download: <日期/時間>`**：顯示上一次「完全成功」的批次**開始時間**。
  這個時間戳只會在批次**全部完成、沒有任何失敗檔案、沒有暫停、也沒有取消**
  之後才會被記錄 — 只要有任何檔案失敗，或你中途取消/暫停，這個標籤都不會更新。
