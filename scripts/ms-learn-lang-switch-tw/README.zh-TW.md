...
此腳本套用於符合 `https://*.microsoft.com/*/*` 的網址，並用以下正規表示式檢查 `location.pathname`：

```
/^\/(en-us|zh-tw)\//i
```

此正則錨定在**第一個路徑區段**，且 `i` 旗標讓比對不分大小寫，因此可比對小寫 `en-us` / `zh-tw` 與大寫 `en-US` / `zh-TW`（`support.microsoft.com` KB／產品文章所用），同時忽略路徑其他位置或 query 中長得像語系碼的片段。若第一個路徑區段不是 `en-us` 或 `zh-tw` 語系，腳本不會做任何事，也不會插入按鈕，因此不會干擾未使用此語系慣例的頁面。

點擊按鈕時，腳本透過 `URL` API 只替換單一語系區段（`en-us` ⇄ `zh-tw`）並導向，其餘路徑以及 query 與 hash 都會保留。
