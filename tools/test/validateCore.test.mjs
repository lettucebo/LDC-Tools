...
// ---------------------------------------------------------------------------
// Off-main baseline poisoning: a tag only proves "some commit somewhere in
// this repository was synchronized at that version". Anyone able to push a
// tag (or any fork/branch whose tags get fetched) could create a perfectly
// synchronized `vX.Y.Z` tag on a commit that was never merged into main.
// Selecting such a tag as the release baseline lets off-main history dictate
// the strictly-greater gate: a bogus high off-main tag would block every real
// release, and off-main content would be treated as released history.
// Baseline candidates must therefore be ancestors of
// refs/remotes/origin/main, and when that ref is missing the check must fail
// closed rather than silently degrade to "any tag anywhere".
// ---------------------------------------------------------------------------

function buildOffMainBaselineFixture() {
    const dir = scratch('release-off-main-baseline');
    const ids = ['alpha', 'beta'];

    writeFixtureScripts(dir, ids, '0.7.0');
    gitInit(dir); // commit 1 on main
    gitTag(dir, 'v0.7.0');
    const firstCommit = gitCurrentSha(dir);

    writeFixtureScripts(dir, ids, '0.8.0');
    gitCommitAll(dir, 'bump to 0.8.0');
    gitTag(dir, 'v0.8.0'); // the only legitimate baseline: on main, synchronized

    writeFixtureScripts(dir, ids, '0.9.0');
    gitCommitAll(dir, 'bump to 0.9.0 (current, being released)');
    gitSetFakeOriginRef(dir, 'main', 'HEAD');

    // Side branch off the first commit: a fully synchronized v0.8.9 tag that
    // was never merged into main. It passes every content-based check and has
    // a higher SemVer than v0.8.0, so only the ancestry filter can exclude it.
    gitRun(dir, ['checkout', '--quiet', '-b', 'side', firstCommit]);
    writeFixtureScripts(dir, ids, '0.8.9');
    gitCommitAll(dir, 'off-main synchronized 0.8.9');
    gitTag(dir, 'v0.8.9');
    gitRun(dir, ['checkout', '--quiet', 'main']);

    return { dir, ids };
}

test('findReleaseBaseline ignores a synchronized tag that is not an ancestor of origin/main', () => {
    const { dir, ids } = buildOffMainBaselineFixture();
    const result = findReleaseBaseline(dir, ids, 'v0.9.0');
    assert.equal(result.ok, true, result.error);
    assert.equal(result.baseline.tag, 'v0.8.0', 'off-main v0.8.9 must never become the baseline');
    assert.equal(result.baseline.version, '0.8.0');
});

test('runReleaseModeCheck passes against an off-main higher tag because that tag is excluded from the baseline', () => {
    const { dir, ids } = buildOffMainBaselineFixture();
    const result = runReleaseModeCheck(dir, ids, '0.9.0', 'v0.9.0');
    assert.equal(result.ok, true, result.errors?.join('\n'));
    assert.equal(result.baseline.tag, 'v0.8.0');
...
test('findReleaseBaseline fails closed when refs/remotes/origin/main is missing', () => {
    const dir = scratch('release-no-origin-main');
    const ids = ['alpha'];
    writeFixtureScripts(dir, ids, '0.8.0');
    gitInit(dir);
    gitTag(dir, 'v0.8.0');
    writeFixtureScripts(dir, ids, '0.9.0');
    gitCommitAll(dir, 'bump to 0.9.0');
    // Intentionally no refs/remotes/origin/main.

    const result = findReleaseBaseline(dir, ids, 'v0.9.0');
    assert.equal(result.ok, false, 'must not fall back to unfiltered tag selection');
    assert.match(result.error, /origin\/main/);
});

test('runReleaseModeCheck fails closed when refs/remotes/origin/main is missing', () => {
    const dir = scratch('release-mode-no-origin-main');
    const ids = ['alpha'];
    writeFixtureScripts(dir, ids, '0.8.0');
    gitInit(dir);
    gitTag(dir, 'v0.8.0');
    ...
    const result = runReleaseModeCheck(dir, ids, '0.9.0', 'v0.9.0');
    assert.equal(result.ok, false);
    assert.ok(result.errors.some((e) => /origin\/main/.test(e)), JSON.stringify(result.errors));
});
